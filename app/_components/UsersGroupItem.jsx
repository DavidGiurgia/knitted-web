...
const UsersGroupItem = ({ users }) => {
  if (!users || users.length === 0) {
    return null; // Dacă nu există utilizatori, nu afișăm nimic
  }

  const displayedUsers = users.slice(0, 2); // Limităm numărul de utilizatori afișați
  const remainingCount = users.length - 2; // Calculăm câți utilizatori nu sunt afișați
